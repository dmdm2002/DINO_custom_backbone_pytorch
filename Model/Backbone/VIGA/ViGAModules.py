...
from Model.Backbone.VIG.gcn_lib.dense.torch_vertex import DynConv2d
...
class GrapherModule(nn.Module):
    def __init__(self, in_channel, hidden_channel, k=9, dilation=1, drop_path=0.0):
        ...
        self.sam = nn.MultiheadAttention(hidden_channel, 8)
    ...
    def forward(self, x):
        ...
        --> 각 pixel 을 node로 사용하여 관계를 학습할 수 있을 것이다.
        """
        ...
        y = x.squeeze(-1)
        y = y.permute((0, 2, 1))
        y, _attention_weight = self.sam(y, y, y)
        y = y.permute((0, 2, 1))
        x = y.unsqueeze(-1)
